...
[1.2 Token 包装格式](#_1-2-token-包装格式)
...
[3. 快速测试【推荐】](#_3-快速测试【推荐】)

[4. 手工测试（直连设备）](#_4-手工测试-直连设备)
...
### [#](#_1-2-token-包装格式) 1.2 Token 包装格式
...
UDP 消息统一使用 IotDeviceMessage 对象，通过 IotMessageSerializer 进行序列化/反序列化。序列化方式通过 `serialize` 配置项指定，详见 [《设备接入（概述）》](https://cloud.iocoder.cn/iot/protocol-overview/) 的「4.3 消息序列化」章节。
...
| --- | --- | --- |
| `auth` | 设备认证 | 否 |
| `thing.auth.register` | 设备动态注册（一型一密），详见 [《设备动态注册》](https://cloud.iocoder.cn/iot/device-register/) | 否 |
...
## [#](#_4-手工测试-直连设备) 4. 手工测试（直连设备）

UDP 没有类似 Postman、curl 这样开箱即用的调试工具，因此暂不提供手工测试案例。**请使用第 3 节的集成测试类进行测试**，它已经封装好了序列化和 Token 包装逻辑。

[设备接入（TCP 协议）](https://cloud.iocoder.cn/iot/protocol-tcp/) [设备接入（WebSocket 协议）](https://cloud.iocoder.cn/iot/protocol-websocket/)
