> 来源：[菜鸟教程 - Python 智能体环境配置](https://www.runoob.com/ai-agent/ai-agent-python-setup.html)
...
# 定义技能
skill = kernel.import_skill_from_directory("./skills", "ExampleSkill")
...
本章节介绍了 Python 智能体开发环境的完整配置过程：

1.  **Python 版本选择**：推荐 Python 3.9 或更高版本
2.  **虚拟环境**：使用 uv 或 venv 隔离项目依赖
3.  **必备库安装**：包括 LangChain、OpenAI、向量数据库等核心库
4.  **API Key 管理**：使用 .env 文件安全管理敏感信息
5.  **开发工具**：VS Code、Git、pytest、Docker 等

完成环境配置后，就可以开始构建 AI Agent 了。
