> 来源：[菜鸟教程](https://www.runoob.com/ai-agent/opencode-coding-plan.html)
...
-   **apiKey**：将 YOUR\_API\_KEY 替换为 [Coding Plan](https://bailian.console.aliyun.com/cn-beijing/?userCode=i5mn5r7m&tab=coding-plan#/efm/detail) 专属 API Key，格式为 sk-sp-xxx。
...
## 订阅方舟 Coding Plan（字节跳动）

访问方舟 [Coding Plan 活动](https://www.volcengine.com/activity/codingplan)，按需订阅套餐，用量不大用 Lite 版本就可以了，如果用量非常大可以使用 Pro 版本。
...
-   **`<ARK_API_KEY>`**：[获取您的 API Key](https://console.volcengine.com/ark/region:ark+cn-beijing/apikey)
-   **`model`**：替换为需要使用的模型名称
